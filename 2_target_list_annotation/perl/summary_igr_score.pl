...
#### Author: Shengdi Li ####
#### 2022 Nov. 2 ####
#### Summarizing IGR scores from the IGR program output ####
use Getopt::Long;
use strict;
...
if($display_help)
{
	print "Command:\n\tperl summary_igr_score.pl [-h] -in INPUT_TARGET_LIST -workdir PATH_TO_TRL_OUTPUT -out OUTPUT_SUMMARY -w WINDOW_SIZE\n\n";
...
while(my $read_line=<INDEX>)
{
	...
	open(TMP,$data_dir."/".$id.".igr".$w.".txt");
	my $line=<TMP>;
	$line=<TMP>;
	chomp $line;
